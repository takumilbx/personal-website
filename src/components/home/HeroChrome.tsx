    ...
    <>
      <header className="absolute inset-x-0 top-0 z-30 flex items-start justify-between px-6 pt-6 sm:px-10 sm:pt-8">
        ...
        <div className="hidden items-start gap-16 sm:flex lg:gap-24">
          ...
          <nav aria-label="Site" className="flex flex-col gap-0.5 text-sm">
            {nav.map((l, i) => (
              <span key={l.href} className="anim-fade-up" style={{ animationDelay: `${1000 + i * 80}ms` }}>
                <a href={l.href} className="transition-opacity duration-300 hover:opacity-60">{l.label}</a>
              </span>
            ))}
          </nav>
          <nav aria-label="Elsewhere" className="flex flex-col gap-0.5 text-sm">
            {social.map((l, i) => (
              <span key={l.href} className="anim-fade-up" style={{ animationDelay: `${1150 + i * 80}ms` }}>
                <a href={l.href} target="_blank" rel="noopener" className="transition-opacity duration-300 hover:opacity-60">{l.label}</a>
              </span>
            ))}
          </nav>
        </div>
      ...
      <aside
        id="site-drawer"
        aria-hidden={!open}
        className={`fixed inset-y-0 right-0 z-40 w-[80%] max-w-sm bg-panel px-8 py-10 transition-transform duration-[600ms] sm:hidden ${open ? 'translate-x-0' : 'translate-x-full'}`}
      ...
      >
        ...
        <p className="text-xs uppercase tracking-[0.2em] text-cream/50 transition-[opacity,transform] duration-500" style={reveal(250, 12)}>{navLabel}</p>
        <nav aria-label="Site" className="mt-4 flex flex-col gap-2">
          {nav.map((l, i) => (
            <a key={l.href} href={l.href} tabIndex={open ? 0 : -1} className="text-4xl text-cream transition-[opacity,transform] duration-500" style={reveal(300 + i * 80, 24)}>{l.label}</a>
          ))}
        </nav>

        <p className="mt-10 text-xs uppercase tracking-[0.2em] text-cream/50 transition-[opacity,transform] duration-500" style={reveal(500, 12)}>{socialLabel}</p>
        <div className="mt-3 flex flex-wrap gap-x-5 gap-y-2 text-sm">
          {social.map((l, i) => (
            <a key={l.href} href={l.href} target="_blank" rel="noopener" tabIndex={open ? 0 : -1} className="text-cream transition-[opacity,transform] duration-500" style={reveal(550 + i * 60, 16)}>{l.label}</a>
          ))}
        </div>
      </aside>
    </>
